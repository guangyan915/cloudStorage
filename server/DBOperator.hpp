...
public:
  ...
  std::vector<UserInfo> getFriendList(std::string name);
  UserInfo getUserInfo(std::string id);
  bool deleteFriend(std::string name1, std::string name2);
};
...
bool DBOperator::setAllUserOntOnline() {
  std::string sql = "update userInfo set online = 0";
...
bool DBOperator::insertNotOnlineMessage(int recv_id, int send_id, int message_type, const std::string &message) {
  std::string sql;
  if(message.size() == 0) sql = QString("insert into notNolineMessage (recv_id, send_id, message_type, send_time) value(%0, %1, %2,now())").arg(std::to_string(recv_id)).arg(std::to_string(send_id)).arg(std::to_string(message_type)).toStdString();
  else sql = QString("insert into notNolineMessage (recv_id, send_id, message_type, message, send_time) value(%0, %1, %2, '%3', now())").arg(std::to_string(recv_id)).arg(std::to_string(send_id)).arg(std::to_string(message_type)).arg(message).toStdString();
  std::cout << sql << std::endl;
  return MySQL::getInstance().exec(sql);
}

std::vector<UserInfo> DBOperator::getFriendList(std::string name) {
  int id = getUserId(0, name);
  std::string sql = QString("select * from friendInfo where id1 = %0 or id2 = %1").arg(std::to_string(id)).arg(std::to_string(id)).toStdString();
  std::cout << sql << std::endl;
  std::vector<UserInfo> res;
  auto e = MySQL::getInstance().query(sql);
  for(auto& a : e) {
    UserInfo user_info = {0};
    if(a[0] == std::to_string(id)) {
      user_info = getUserInfo(a[1]);
    }
    else {
      user_info = getUserInfo(a[0]);
    }
    res.push_back(user_info);
    //std::cout << res.back() << std::endl;
  }
  return res;
}

UserInfo DBOperator::getUserInfo(std::string id) {
  std::string sql = QString("select * from userInfo where id = %0").arg(id).toStdString();
  auto res = MySQL::getInstance().query(sql);

  UserInfo user_info = {0};
  if(res.empty()) return user_info;
  user_info.id = atoi(res[0][0].c_str());
  strcpy(user_info.name, res[0][1].c_str());
  user_info.online = res[0][3][0];
  return user_info;
}


bool DBOperator::deleteFriend(std::string name1, std::string name2)  {
  int id1 = getUserId(0, name1);
  int id2 = getUserId(0, name2);
  
  if(id1 < 0 || id2 < 0) return false;
  std::string sql = QString("delete from friendInfo where id1 = %0 and id2 = %1").arg(std::to_string(id1)).arg(std::to_string(id2)).toStdString();
  std::cout << sql << std::endl;
  MySQL::getInstance().exec(sql);
  sql = QString("delete from friendInfo where id1 = %0 and id2 = %1").arg(std::to_string(id2)).arg(std::to_string(id1)).toStdString();
  MySQL::getInstance().exec(sql);
  std::cout << sql << std::endl;
  return true;
}
